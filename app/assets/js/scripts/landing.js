...
const refreshServerStatus = async function(fade = false){
    ...
    try {
        ...
        loggerLanding.warn('Impossible de rafraichir les status des serveurs. Ils sont peut-être hors-ligne.')
        loggerLanding.debug(err)
    }
    if(fade){
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    'Aucune installation<br>de Java compatible trouvée.',
                    'Pour rejoindre Nercoria, vous avez besoin de Java 8 64 bits. En installant Java, vous acceptez les <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">termes de license Oracle.</a>.',
                    'Installer Java',
                    'Installer Manuellement'
                )
                setOverlayHandler(() => {
                    setLaunchDetails('Preparatioàn du télechargement...')
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            ...
                            'Une installation valide de Java 8 est requise.',
                            'Je comprends.',
                            'Revenir en arrière.'
                        )
                        setOverlayHandler(() => {
            ...
            if(m.result === true){
                ...
                setLaunchDetails('Télechargement de Java..')
                ...
                setOverlayContent(
                    ...
                    'Unfortunately we\'ve encountered an issue while attempting to install Java. You will need to manually install a copy. Please check out our <a href="https://github.com/valkam08/NercoriaLauncher/wiki">Troubleshooting Guide</a> for more details and instructions.',
                    'I Understand'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
            ...
            switch(m.data){
                ...
                // Download & extraction complete, remove the loading from the OS progress bar.
                    ...
                    setLaunchDetails('Java a bien été installé.')
...
function dlAsync(login = true){
    ...
    if(login) {
        if(ConfigManager.getSelectedAccount() == null){
            loggerLanding.error('Vous devez être connecté.')
            return
        }
    }
        ...
        if(m.context === 'validate'){
            switch(m.data){
                case 'distribution':
                    ...
                    loggerLaunchSuite.log("Vérification de l'index.")
                    setLaunchDetails('Chargement des informations sur la version...')
                    break
                case 'version':
                    setLaunchPercentage(40, 100)
                    loggerLaunchSuite.log('Informations sur la version chargées.')
                    setLaunchDetails("Vérification de l'intégrité des assets...")
                    break
                case 'assets':
                    setLaunchPercentage(60, 100)
                    loggerLaunchSuite.log('Vérification finie.')
                ...
                case 'files':
                    ...
                    loggerLaunchSuite.log('Vérification compléte.')
            ...
            switch(m.data){
                case 'download':
                    loggerLaunchSuite.error('Erreur pendant le télechargement:', m.error)
                    
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            'Erreur',
                            'Connexion impossible aux serveurs de télechargement. Verifiez votre connexion à Internet puis réessayez.'
                        )
                    } else {
                        showLaunchFailure(
                            'Erreur',
            ...
            if(m.result.forgeData == null || m.result.versionData == null){
                loggerLaunchSuite.error('Erreur pendant le vérification:', m.result)

                loggerLaunchSuite.error('Erreur pendant le lancement.', m.result.error)
                showLaunchFailure('Une erreur est survenue pendant le lancement.', 'Voir la console (CTRL + Shift + i) pour plus de détails.')

                allGood = false
            }

            fs.remove(this.gameDir + "/nercoria-1.7.10/mods", function(err) {
                if (err) throw err;
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Sur le serveur !')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Joue à Nercoria!')
                    }
                }
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1){
                        ...
                        showLaunchFailure('Error During Launch', 'The main file, LaunchWrapper, failed to download properly. As a result, the game cannot launch.<br><br>To fix this issue, temporarily turn off your antivirus software and launch the game again.<br><br>If you have time, please <a href="https://github.com/valkam08/NercoriaLauncher/issues">submit an issue</a> and let us know what antivirus software you use. We\'ll contact them and try to straighten things out.')
                    }
                }
